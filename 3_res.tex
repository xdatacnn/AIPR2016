...
\subsection{Tensor and NVIDIA DevBox}
...
%review repetition in methods
The CNN we use is adapted from the AlexNet provided in TensorFlow []. It consists of two convolutional layers, with 64 filter with size 5x5, with no strides and no zero
padding. Two layers of max pooling immediately follow the first two convolutional
layers, with size 2x2 and no stride. The last layer is fully-connected and outputs a feature vector, which is ultimately used as signature to retrieve images by similarity.
